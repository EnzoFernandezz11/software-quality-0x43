...
namespace sensors
{
    ...
    public:
        ...
        /**
         * @brief Samples the PIR and latches any detected motion.
         *
         * Must be called frequently from the main loop. The PIR keeps its
         * output HIGH for only a few seconds, so sampling it once per
         * telemetry interval would miss most motion events. This latches a
         * flag that read() consumes and clears.
         */
        void pollMotion();
         ...
         *
         * Reports motion latched since the previous call and clears the
         * latch for the next interval.
         *
        ...
        /**
         * @brief Returns the key currently pressed on the 4x4 keypad.
         *
         * @return char Pressed key, or '\0' if none.
         */
        char readKey() const;

    private:
        const app::AppConfig& config_;
        bool motionLatched_ = false;
    };
